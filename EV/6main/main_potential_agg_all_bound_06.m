...
for sim_idx = 1:length(incentive_prices)
    ...
    [EVs, t_sim, ~, ~, P_tar] = initializeFromExcel_8am_diffinc(excelFile);
    fprintf('成功加载%d辆EV数据\n', length(EVs));
    ...
    %% 结果保存
    % 根据当前价格生成唯一文件名
    outputFileName = sprintf('results_incentive_%.2f_1000_bound_06.mat', current_incentive_price);
    fprintf('\n正在保存结果到 %s ...\n', outputFileName);
    try
