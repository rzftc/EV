...
if exist('results', 'var')
    ...
    % 保存图像 (无标题，高DPI，中文名)
    print(fig1, '功率跟踪效果分析.svg', '-dsvg');
end
%% --------------------------------------------------
% [修改] 图 2: 聚合模型实时功率与实际统计功率对比 (拆分为全景图与局部放大图)
% 保存为: 聚合功率计算验证_全景.svg, 聚合功率计算验证_放大.svg
% --------------------------------------------------
if exist('results', 'var') && isfield(results, 'EV_Power')
    ...
    % 保存全景图
    print(fig2_main, '聚合功率计算验证_全景.svg', '-dsvg');
    % --- 3. 绘制并保存局部放大图 (Child Plot) ---
    % 【修改】保存为 SVG 矢量格式，并去除网格线
    ...
    % === 关键修改：保存为 SVG 矢量图 ===
    % 设置属性以确保 SVG 转换时不带背景色
    set(gcf, 'Color', 'none'); 
    set(gca, 'Color', 'none');
    set(gcf, 'InvertHardcopy', 'off'); 
    % 保存放大图
    print(fig2_zoom, '聚合功率计算验证_放大.svg', '-dsvg');
else
    if exist('results', 'var')
...
if exist('results', 'var')
    ...
    if selected_ev > size(results.EV_S_original, 1)
        ...
        % 保存图像 (无标题，高DPI，中文名)
        print(fig3, '单体SOC与Lambda.svg', '-dsvg');
    end
end
%% --------------------------------------------------
...
if exist('results', 'var')
    ...
    if ~isfield(results, 'P_base_agg') || ~isfield(results, 'EV_Up')
        ...
        % 保存图像
        print(fig6, '聚合调节边界.svg', '-dsvg');
    end
end
%% --------------------------------------------------
% 图 7: 不同调节时长上调节能力对比 (5min vs 15min vs 60min)
% 保存为: 不同调节时长上调节能力对比.svg
% --------------------------------------------------
...
if file_found_count > 0
    ...
    % 保存图像
    print(fig7, '不同调节时长上调节能力对比.svg', '-dsvg');
else
...
if file_found_count > 0
    ...
    legend('Location', 'west', 'FontSize', 16);
    % 保存图像
    print(fig8, '不同调节时长下调节能力对比.svg', '-dsvg');
else
