...
if ~isempty(group_indices)
    ...
    % 字体设置
    xlabel('时间 (小时)', 'FontSize', 20, 'FontName', 'SimHei');
    ylabel('归一化电量偏差度', 'FontSize', 20, 'FontName', 'SimHei');
    grid on;
    box on;
    set(gca, 'FontSize', 16);
    
    % 图例 (字体调小)
    legend([h_legend_ind, h_agg, h_lambda], 'Location', 'best', 'FontSize', 16, 'FontName', 'SimHei');
