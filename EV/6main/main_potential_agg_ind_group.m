...
% 功能：计算并对比“单体求和”与“聚合模型”的调节潜力，包含分组聚合改进。
% 特性：支持特定同质化参数组的微观-宏观响应对比（S' vs Lambda），并验证聚合模型实时功率。
clc; clear; close all;
rng(2024);
...
fprintf('仿真时间范围: %.2f - %.2f 小时, 步长: %.3f 小时 (%d 点)\n', ...
...
results = struct(...
    ...
    'P_agg_ptcp',     zeros(1, total_steps), ... % 参与聚合EV的总功率
    ...
    ...
    'EV_E_actual',        zeros(num_evs, total_steps), ... % 实际电量轨迹
    'EV_E_baseline',      zeros(num_evs, total_steps), ... % 基线电量轨迹
...
% 扩展结果结构体 (存储组聚合S和S')
results.S_agg_group_raw = zeros(1, total_steps);    
results.S_agg_group_prime = zeros(1, total_steps); 
        ...
        if ~isempty(active_participating_indices)
            ...
            for g = 1:length(group_edges)-1
                ...
                if ~isempty(group_indices_in_agg)
                    ...
                    agg_P_real = agg_P_real + sum([group_EVs.P_current]);
                end
            end
        end
...
%% 绘图: 同质化单体 S' vs 聚合 S' vs Lambda
if ~exist('group_indices', 'var') || isempty(group_indices)
    warning('未找到 group_indices，尝试重新筛选...');
...
if ~isempty(group_indices)
    ...
    % Layer 2: 聚合平均 S' (蓝色粗实线)
    h_agg = plot(time_points_absolute, results.S_agg_group_raw, ...
        'Color', 'b', 'LineWidth', 2.5, 'DisplayName', '聚合均值电量偏差度');
        
    % Layer 3: Lambda (红色粗虚线)
    h_lambda = plot(time_points_absolute, results.lambda, ...
        'Color', 'r', 'LineStyle', '--', 'LineWidth', 2.5, 'DisplayName', 'lambda');
    ...
    xlabel('时间 (小时)', 'FontSize', 22);
    ylabel('归一化电量偏差度', 'FontSize', 22);
    grid on; box on;
    
    set(gca, 'FontSize', 18);
    legend([h_legend_ind, h_agg, h_lambda], 'Location', 'best', 'FontSize', 18);
